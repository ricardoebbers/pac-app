defmodule PacWeb.DocumentLive.FormComponent do
  ...
  def update(%{document: document} = assigns, socket) do
    changeset = Core.change_document(document)
  ...
  def handle_event("validate", %{"document" => document_params}, socket) do
    changeset =
      ...
      |> Core.change_document(document_params)
  ...
  defp save_document(socket, :edit, document_params) do
    case Core.update_document(socket.assigns.document, document_params) do
      {:ok, document} ->
  ...
  defp save_document(socket, :new, document_params) do
    case Core.create_document(document_params) do
      {:ok, document} ->
        notify_parent({:saved, document})
